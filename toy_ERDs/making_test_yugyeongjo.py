...
# 출제받은 문제 DB에 넣기
def test_db_insert(test_list, option_count,conn): # 앞서 실행 된 making_test_from_file 펑션의 리턴값인 문제 리스트와, 문항 당 보기 수를 매개변수로 동작 선언
    import pymysql
    ...
    try:
        with conn.cursor() as cursor:
            ...
            cursor.execute(sql)
            conn.commit()
            sql = "DELETE FROM USERS WHERE USERS_ID IS NOT NULL"
            cursor.execute(sql)
            conn.commit()
            sql = "DELETE FROM QUIZS WHERE PK_QUIZ_ID IS NOT NULL"
            cursor.execute(sql)
            conn.commit()

            # 매개 변수로 받은 시험 문제와 보기의 리스트를 DB에 입력하는 반복문 실행
            for i in range(len(test_list)): # 리스트 내 딕셔너리 갯수 만큼 반복문 실행
                sql = "SELECT COUNT(PK_QUIZ_ID) FROM `QUIZS`"
                cursor.execute(sql)
                QUIZ_number = cursor.fetchall()
                QUIZ_number = int(QUIZ_number[0][0])

                sql = "INSERT INTO QUIZS (PK_QUIZ_ID, CONTENT, POINTS, CORRECT, NUMBERS, FK_QUIZ_ID) VALUES ('QUIZ_%s', %s, %s, %s, %s,'QUIZ_%s')"
                cursor.execute(sql, (QUIZ_number+1, test_list[i]["question"], test_list[i]["point"],None, i+1 , QUIZ_number+1))
                conn.commit()
                for j in range(option_count):  # 매개 변수로 받은 보기 문항 갯수 만큼 반복문 실행
                    ...
                    sql = "INSERT INTO QUIZS (PK_QUIZ_ID, CONTENT, POINTS, CORRECT, NUMBERS, FK_QUIZ_ID) VALUES ('QUIZ_%s', %s, %s, %s, %s,'QUIZ_%s')"
                    cursor.execute(sql, (option_number+1, test_list[i]["option"][j], 0, test_list[i]["correct"][j], j+1,QUIZ_number+1))
                    conn.commit()               
    finally:
        conn.close()       
        
    return
test_db_insert(test_list, option_count,conn)
